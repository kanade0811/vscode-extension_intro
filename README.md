...
8. [この講座の単位認定について](#8この講座の単位認定について)
...
![Node.jsのDL画面](./images/node.png)

> [!NOTE]
> 画面に**LTS**、**npm**が表示されていることを確認してDLしましょう。  
> その他の設定はそのままで大丈夫です。
...
// 今回はTypeScriptの場合で説明するので1番上
? What's the identifier of your extension?
// そのままエンター押せば何とかなる(らしい)
? Initialize a git repository?
// gitリポジトリを作るならここ
// (既にgitにいるのにyと答えると二重になるため注意by前科1犯)
...
以下が今回主に説明する、最低限知っておいた方が良いファイルです
...
> モジュールの追加等々するときに編集したりします(詳しくは自分で調べてください)
...
VScodeの一番左にあるバーの上から5つ目、正方形4つの`拡張機能`をクリックし開きます。  
...
ではインポートされているか確かめてみましょう。

VScodeを再起動し、拡張機能タブの三点リーダーから`実行中の拡張機能の表示`を選択してください。  
すると中に、先ほど作った名前の拡張機能が入っているはずです。  
実際に、先ほど拡張機能ホストで動かしたように`Ctrl`+`Shift`+`P`でコマンドパレットを開き、`Hello World`と打ち選択してみましょう。拡張機能ホストと同じ`Hello World from (拡張機能名)!`という通知が右下に表示されるはずです。



## 8.この講座の単位認定について

この講座の単位認定条件は、以下のように**空のVScodeで、右下の通知に`Hello World from (拡張機能名)!`が表示されていると分かるスクリーンショット**を、Googleフォームに送ってもらいます。  

![単位認定のスクショ](./images/unit.png)  

と言っても、難しく考える必要はありません。もしできないようでしたら私(とかチャッピーとかに)聞いてくださいね