...
// どういう型で作りたい？ってやつ。今回はTypeScriptの場合で説明するので1番上のまま`Ender`
...
> [!WARNING]
> 先ほど作成したフォルダ以外が親となると、のちに実行できなくなるので注意  
...
> [!WARNING]
> その他のファイルやフォルダも勿論必要ですので、**絶対に**消さないでください
...
これで実際に動かせる環境は揃いました！  
`Fn`+`F5`を押せば「拡張機能開発ホスト」という、今作ってる拡張機能が実際に動くような新しいVScodeのウィンドウが表示されます。
...
中身をどう作るかは今回の講座の範囲外なので、自分で調べてやりたいようにやってください。
...
> [!NOTE]
> この`README.md`は本来、名前・説明・現在のバージョンや既出バグなどを書き込む場所です。
> しかし、(現状が伝わらないだけで)拡張機能本体には関係ないのでご安心を。
...
`DONE`と表示されれば、先ほどまでいたフォルダに`(ファイル名)-0.0.1.vsix`というファイルが作られていると思います。  



## 7.拡張機能のインポート

ここで、先ほどエクスポートしたファイルをインポートすることで、どのウィンドウでも拡張機能が使えるようにしましょう。
